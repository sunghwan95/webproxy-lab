...
// 포트번호를 인자로 받아서 해당 포트로 들어오는 요청을 처리하는 함수.
...
// 클라이언트의 요청을 처리하는 함수
// 요청을 분석하여 요청 대상 결정 -> 원격 서버로 요청 전송 -> 응답을 클라이언트에게 전송
void *thread(void *vargp){
...
void doit(int clientfd){
  ...
  // Server 소켓 생성
  serverfd = is_local_test ? Open_clientfd(hostname, port) : Open_clientfd("3.39.10.157", port);
  if (serverfd < 0){
...
// uri를 파싱하여 `hostname`, `port`, `path`로 추출하는 함수
...
void read_requesthdrs(rio_t *request_rio, void *request_buf, int serverfd, char *hostname, char *port){
  ...
  Rio_readlineb(request_rio, request_line, MAXLINE); // 첫번째 줄 읽기
  while (strcmp(request_line, "\r\n")){
    if (strstr(request_line, "Proxy-Connection") != NULL){
      sprintf(request_buf, "Proxy-Connection: close\r\n");
      is_proxy_connection_exist = 1;
    }else if (strstr(request_line, "Connection") != NULL){
    ...
    Rio_readlineb(request_rio, request_line, MAXLINE);       // 다음 줄 읽기
  }
