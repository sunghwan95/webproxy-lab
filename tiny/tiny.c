...
void doit(int fd){
  ...
  if(stat(filename, &sbuf) < 0){ // 파일이 디스크에 없으면
    clienterror(fd, filename, "404", "Not found", "Tiny couldn't this file");
    return;
  }
...
void serve_dynamic(int fd, char* filename, char* cgiargs){
  ...
  if(Fork() == 0){ // 자식 프로세스 포크
    ...
    Execve(filename, emptylist, environ); // 현재 프로세스으 이미지를 filename 프로그램으로 대체
  }
  Wait(NULL);
}